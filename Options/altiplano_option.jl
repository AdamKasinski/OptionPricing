...
function price_altiplano_moment_matching(num_of_sim::Int,α::Float64,T::Int,N::Int, treshold::Float64, r::Float64, K::Float64, C::Float64, 
                        basket_volume::Int, S₀::Array{Float64},  sigma::Array{Float64}, correlation_matrix::Matrix{Float64})
    dt::Float64 = T/N
...
function altiplano_option_monte_carlo(num_of_sim::Int, α::Float64, T::Int, N::Int, treshold::Float64, 
    ...
    if method == "quasi_monte_carlo"
        return price_altiplano_quasi_monte_carlo(T, N, treshold, r, K, C, basket_volume, S₀, sigma, 
                                                                                    correlation_matrix)
    elseif method == "basic"
        rtrn = [price_altiplano_normal(T, N, treshold, r, K, C, basket_volume, S₀, sigma, 
                                        correlation_matrix) for iteration in 1:num_of_sim]
    elseif method == "antithetic"
        rtrn = [price_altiplano_antithetic_variates(T,N, treshold, r, K, C, basket_volume, S₀, sigma, 
                                                    correlation_matrix) for iteration in 1:num_of_sim]
    elseif method == "LHS"
        rtrn = [price_altiplano_LHS(T,N, treshold, r, K, C, basket_volume, S₀, sigma, 
                                    correlation_matrix) for iteration in 1:num_of_sim]
    else
